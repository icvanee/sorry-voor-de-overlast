...
class PlanningVersion:
    ...
    def set_final(version_id):
        ...
        if Config.DB_TYPE == 'postgresql':
            ...
            # First, unmark all other versions
            conn.execute('UPDATE planning_versions SET is_active = FALSE')
            # Mark this version as final
            conn.execute('UPDATE planning_versions SET is_active = TRUE WHERE id = ?', (version_id,))
            conn.commit()
        ...
        if Config.DB_TYPE == 'postgresql':
            ...
            if pinned_matches:
                ...
                cursor.execute(f'''
                    INSERT INTO match_planning (planning_version_id, match_id, player_id, is_confirmed, actually_played, is_pinned, notes)
                    SELECT ?, match_id, player_id, is_confirmed, actually_played, 
                           CASE WHEN match_id IN ({pinned_placeholders}) THEN 1 ELSE 0 END, notes
                    FROM match_planning 
                    WHERE planning_version_id = ?
                ''', [new_version_id] + pinned_matches + [source_version_id])
            else:
                cursor.execute('''
                    INSERT INTO match_planning (planning_version_id, match_id, player_id, is_confirmed, actually_played, is_pinned, notes)
                    SELECT ?, match_id, player_id, is_confirmed, actually_played, 0, notes
                    FROM match_planning 
                    WHERE planning_version_id = ?
                ''', (new_version_id, source_version_id))
    ...
    def get_final():
        ...
        if Config.DB_TYPE == 'postgresql':
            ...
            version = conn.execute('''
                SELECT * FROM planning_versions WHERE is_final = 1 AND deleted_at IS NULL
            ''').fetchone()
